...
PORTS =                 c(:port_start, 3000)..c(:port_end, 3020)
  ...
  class AcceptanceTestRunner
    ...
    private
      ...
      def log_file browser
        ...
        #Saves the results in folder with name convention: today'sDate_nameOfBrowser
        today = Date.today.strftime("%d_%m_%G")
        filename_today = today + "_" +  browser + '.yml'
        #Overwrites results if they are conduced more then once a day
        File.exist?(log_path(filename_today)) ? File.delete(log_path(filename_today)) : '' 
      ...
      def print_result(result, browser)
        ...
        #Removes results from 6 days ago
        last_week = Date.today-6
        last_week = last_week.strftime("%d_%m_%G")
        result_last_week = last_week + "_" +  browser
        
        #directory storing the old results (created 6 days ago)
        directory_to_be_deleted = result['resultDir'][0,result['resultDir'].size-result_last_week.size] + result_last_week
        
        File.exist?(directory_to_be_deleted) ? FileUtils.remove_dir(directory_to_be_deleted) : ''
      end
