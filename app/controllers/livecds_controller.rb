...
class LivecdsController < ApplicationController
  ...
  def index
    if params[:id]=="show" then
      ...
      if(!params[:cd].nil?)
        @livecds = Livecd.scoped.where(:failed => false, :generated => true)
        @livecds = @livecds.where(:distribution_id => params[:cd][:distribution]) if !params[:cd][:distribution].blank?
        @livecds = @livecds.where(:derivative_id => params[:cd][:derivative]) if !params[:cd][:derivative].blank?
        @livecds = @livecds.where(:architecture_id => params[:cd][:architecture]) if !params[:cd][:architecture].blank?

        @livecds = @livecds.paginate(:page => params[:page], :per_page => 15).order('id DESC')
      else
        @livecds = Livecd.paginate(:page => params[:page], :per_page => 15).order('id DESC')
      end
    end  
  end
